...
public class Controller extends SearchFunctions implements Initializable {
    ...
    public void handleSearchClassroomNow(ActionEvent actionEvent) {
        ...
        ArrayList<Classroom> busyClassroom = classroombusy(new DateSchedule(14, 30, 5), nodesRB);
    ...
    public void handleShowClassroomClassNow(ActionEvent actionEvent) {
        if (classroomNowSearch.getText() != null) {
            ...
            Lesson l = c.classroomstate(new DateSchedule(14, 30, 5));
    ...
    public void handleShowLessonsByInterval(ActionEvent actionEvent) {
        ...
        if (nodesRB.contains(classroom)) {
            ...
            if (lessonArrayList.size() > 0) {
                ...
                alert(Alert.AlertType.ERROR, "Error", "Ups, algo correu mal", "Não existe nenhuma sala ocupada nessa hora");
            }
        } else {
            alert(Alert.AlertType.ERROR, "Error", "Ups, algo correu mal", "Não existe nenhuma Sala com esse nome");
        }
    ...
    public void handleInsertLessonButton(ActionEvent actionEvent) {
        ...
        Lesson lesson = new Lesson(lessonsST.size() + 1, startHour, endHour);

        // verifica a existencia da turma recebida
        if (!classST.contains(aClass)) {
            // criação de uma nova turma
            newClass = new Class(aClass, year, size, type, curricularUnitsST.get(ucname));
            ...
            if (!teacherComboBox.isDisable()) {
                ...
                for (Long key : teachersST.keys()) {
                    if (teachersST.get(key).getName().equals(teacherName)) {
                        ...
                        if (!(newClass.addTeacher(teachersST.get(key)))) {
            ...
            classST.put(newClass.getId(), newClass);
            // associação da aula a sala
            if ((!classroom.addLesson(lesson))) {
    ...
    public void handleChangeLessonClassroomSubmit(ActionEvent actionEvent) {
        ...
        Lesson newLesson = lessonsST.get(lessonid);
        Class aClass = classST.get(newLesson.getaClass().getId());
        Classroom classroom = (Classroom) nodesRB.get(classroomiD);
        if (newLesson.editClassroom(classroom)) {
            aClass.setLesson(newLesson);
            //se a turma tiver professor associado
            if (aClass.getTeacher() != null) {
                Teacher teacher = teachersST.get(aClass.getTeacher().getNumber());
                teacher.deleteClass(aClass.getId());
    ...
    public void handleShowTeacherComboBox(ActionEvent actionEvent) {
        ...
        String[] teacher = new String[uC.getTeachers().size() - 1];
        int i = 0;
        for (Long key : uC.getTeachers().keys()) {
            //se esta turma tiver professor associado
            if (aC.getTeacher() != null) {
                //retorna o ou os professores que lecionam a unidade curricular exceto o que já está associado a essa turma
    ...
    public void handleChangeClassSubmit(ActionEvent actionEvent) {
        ...
        if (sizeChangeClassField.isVisible()) {
            ...
            Teacher teacher = teachersST.get(teacherid);

            //verifica se a turma tem pelo menos uma aula
            if (aC.getLesson() != null) {
                // verifica a possibilidade de adicionar a turma ao professor
                if (teacher.addClass(aC)) {
                    ...
                    alert(Alert.AlertType.ERROR, "Error", "Ups, algo correu mal", "O professor " + teacher.getName() + " está ocupado no horario da aula");
                }
            } else {
    ...
    public void handleAddLessontoClassSubmit(ActionEvent actionEvent) {
        ...
        //Verifica a disponibilidade da sala para a aula criada se estiver disponivel já associa a aula a sala
        if(classroom.addLesson(lesson)){
            // associa a sala à aula.
            lesson.setClassroom(classroom);
        }
    ...
    /**
     * Função que mostra todas as salas disponiveis num intervalo de tempo do separador turmas
     *
     * @param actionEvent
     */
    public void handleGetallClassroomsFree(ActionEvent actionEvent) {
        ...
        // vai percorrer todas as "classrooms", ou seja, todas as salas retornadas que não estão ocupadas entre as horas enviadas
