    ...
    public static ArrayList<FreeClassroom> freeClassroomsbetween(DateSchedule startHour, DateSchedule endHour, RedBlackBST_AED2<String, Node> classroomsST) {
        ...
        for (String idClass : classroomsST.keys()) {
            if (classroomsST.get(idClass) instanceof Classroom) {
                Classroom classroom = (Classroom) classroomsST.get(idClass);

                DateSchedule firstFree = endHour;
                DateSchedule lastFree = startHour;
                FreeClassroom freeclassroom = new FreeClassroom(classroom.getName(), lastFree, firstFree);
                for (Lesson lesson : classroom.getLessons()) {
                    if (lesson.betweendates(startHour, endHour)) {
                        if (lesson.getStarthour().beforeDateShedule(firstFree)) {
                            firstFree = lesson.getStarthour();
                        }
                        if (lesson.getEndhour().afterDateShedule(lastFree)) {
                            lastFree = lesson.getEndhour();
                        }
                    }
                }

                if (firstFree == endHour && lastFree == startHour) {
                    System.out.println("A sala " + classroom.getName() + " está livre entre durante todo o tempo escolhido");
                    ...
                    System.out.println("A sala " + classroom.getName() + " está livre entre: " + startHour + " até " + firstFree + " e entre " + lastFree + " até " + endHour);
                    ...
                    System.out.println("A sala " + classroom.getName() + " está livre entre: " + lastFree + " até " + endHour);
                    ...
                    System.out.println("A sala " + classroom.getName() + " está livre entre: " + startHour + " até " + firstFree);
                    freeclassroom.setEndHour(firstFree);
                    freeClassrooms.add(freeclassroom);
                }
            }
